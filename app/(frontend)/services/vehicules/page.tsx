    ...
    try {
      await fetch("/api/events/requests", {
        ...
        body: JSON.stringify({
          ...
          eventLocation: form.adresse || "À définir",
          guestCount: vehicule.places,
          services: [vehicule.nom],
          description: `Véhicule : ${vehicule.nom} (${vehicule.prix})\nDu : ${form.dateDebut}\nAu : ${form.dateFin || "À définir"}\nAdresse : ${form.adresse || "À définir"}`,
        }),
      });
      setSent(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            <h3 className="font-bold text-[#0c4a6e]">Réserver — {vehicule.nom}</h3>
            <p className="text-xs text-gray-400 mt-0.5">{vehicule.prix} · {vehicule.places} places</p>
          </div>
        ...
        <div className="p-5">
          {sent ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 text-sm mb-6">Notre équipe vous contactera sous 24h pour confirmer la réservation.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="text" value={form.lastName} onChange={e => setForm(f => ({ ...f, lastName: e.target.value }))} placeholder="Kouamé" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c]" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div>
                ...
                <input type="text" value={form.adresse} onChange={e => setForm(f => ({ ...f, adresse: e.target.value }))} placeholder="Hôtel, adresse ou lieu de prise en charge" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c]" />
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button type="submit" disabled={sending} className="flex-1 btn-primary justify-center py-3 disabled:opacity-60">
                  {sending ? <><Loader2 size={15} className="animate-spin" /> Envoi...</> : <>Confirmer la réservation <ArrowRight size={14} /></>}
                </button>
              ...
              <p className="text-xs text-gray-400 text-center">Réponse sous 24h · Sans engagement</p>
            </form>
          )}
        </div>
      </div>
