...
const CATEGORIES = ["Tous", "Festival", "Gala", "Conference", "Fete culturelle", "Salon", "Sport", "Formation"];
    ...
    <div className="min-h-screen">
      ...
      <section className="bg-[#0c4a6e] py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center text-white">
            {[{ value: "17+", label: "Evenements a venir" },{ value: "8", label: "Villes du littoral" },{ value: "25 000+", label: "Participants attendus" },{ value: "100%", label: "Couverture COTIERE" }].map(s => (
      ...
      <section id="evenements" className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <h2 className="section-title mt-2">Evenements a venir</h2>
            <p className="text-gray-500 mt-2">Cliquez sur un evenement pour voir tous les details et vous inscrire.</p>
          </div>
              ...
              return (
                <Link key={ev.slug} href={`/services/rdv/${ev.slug}`}
                  ...
                  <div className="relative h-48 overflow-hidden">
                    <img src={ev.image} alt={ev.titre} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-4 flex items-center gap-1 text-[#c9a84c] text-xs font-semibold group-hover:gap-2 transition-all">
                      Voir les details <ArrowRight size={12} />
                    </div>
                  </div>
                </Link>
              );
      ...
      <section className="py-16 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h2 className="section-title mt-2">Vous organisez un evenement ?</h2>
          <p className="text-gray-500 mt-3 mb-8">Faites connaitre votre evenement sur COTIERE et touchez des milliers de personnes sur tout le littoral ivoirien.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            {[{ icon: Users, title: "Visibilite maximale", desc: "Votre evenement vu par des milliers de personnes sur le littoral" },{ icon: Calendar, title: "Publication rapide", desc: "Mise en ligne sous 24h apres validation de votre demande" },{ icon: Phone, title: "Couverture COTIERE", desc: "Option couverture media : TV, radio, reseaux sociaux" }].map(f => {
              const FIcon = f.icon;
              return (
          ...
          <button
            ...
            <Phone size={18} /> Soumettre mon evenement
          </button>
        </div>
      </section>
      ...
      <section className="py-16 bg-[#0c4a6e] text-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl font-black mb-4">Vous preferez nous appeler ?</h2>
          <p className="text-gray-300 mb-8">Notre equipe est disponible du lundi au samedi de 8h a 18h.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowSoumettreModal(false)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div>
                <h2 className="font-black text-[#0c4a6e] text-lg">Soumettre votre evenement</h2>
                <p className="text-xs text-gray-400 mt-0.5">Mise en ligne sous 24h apres validation</p>
              </div>
            ...
            <div className="p-6">
              {soumSent ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-bold text-[#0c4a6e] mb-2">Soumission envoyee !</h3>
                  <p className="text-gray-500 text-sm mb-6">Notre equipe examinera votre evenement et vous contactera sous 24h.</p>
                ...
                <form onSubmit={handleSoumettreSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" required value={soumForm.nom} onChange={e => setSoumForm(f => ({ ...f, nom: e.target.value }))}
                        placeholder="Jean Kouame" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c]" />
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">Titre de l evenement *</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 mb-1">Date prevue</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">Description de l evenement *</label>
                    <textarea required value={soumForm.description} onChange={e => setSoumForm(f => ({ ...f, description: e.target.value }))}
                      rows={4} maxLength={500}
                      placeholder="Decrivez votre evenement : type, public cible, programme, nombre de participants attendus..."
                  ...
                  <div className="flex gap-3 pt-2">
                    <button type="submit" disabled={soumSending} className="flex-1 btn-primary justify-center py-3 disabled:opacity-60">
                      {soumSending ? <><Loader2 size={15} className="animate-spin" /> Envoi...</> : <>Soumettre mon evenement <ArrowRight size={14} /></>}
                    </button>
