...
export default function AdminAfroubaPage() {
  const [supports, setSupports] = useState<SupportPub[]>([]);
  ...
  async function loadSupports() {
    ...
    try {
      const res = await fetch("/api/afrouba/supports");
      const data = await res.json();
      setSupports(data.data ?? DEFAULT_SUPPORTS);
    } catch {
  ...
  async function saveSupport() {
    ...
    try {
      if (editId) {
        const res = await fetch(`/api/afrouba/supports/${editId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ nom: form.nom, categorie: form.categorie, prix: form.prix, description: form.description, image: form.image }),
        });
        const data = await res.json();
        if (res.ok && data.data) setSupports(prev => prev.map(x => x.id === editId ? data.data : x));
  ...
  async function toggleActive(s: SupportPub) {
    const res = await fetch(`/api/afrouba/supports/${s.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ isActive: !s.isActive }),
    });
    if (res.ok) setSupports(prev => prev.map(x => x.id === s.id ? { ...x, isActive: !x.isActive } : x));
  }

  async function deleteSupport(id: string) {
    if (!confirm("Supprimer ce support ?")) return;
    const res = await fetch(`/api/afrouba/supports/${id}`, { method: "DELETE" });
    if (res.ok) setSupports(prev => prev.filter(x => x.id !== id));
    else alert("Impossible de supprimer ce support.");
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {tab === "catalogue" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {loadingSupports ? (
              <div className="col-span-3 py-12 text-center text-gray-400">Chargement des supports...</div>
            ) : supports.map(s => (
