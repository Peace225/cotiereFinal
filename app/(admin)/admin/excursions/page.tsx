...
export default function AdminExcursionsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      // Réservations génériques de voyage (créées via /api/reservations avec serviceTitle contenant un nom de voyage)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-black text-[#0c4a6e]">Tourisme & Voyage</h1>
                ...
                {excursions.map((e) => (
                  <div key={e.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                    <div className="relative h-40 bg-gray-100">
                      ...
                      <span className={`absolute top-3 left-3 text-xs px-2.5 py-1 rounded-full font-bold ${e.isActive ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
                        {e.isActive ? "DISPONIBLE" : "INACTIF"}
                      </span>
                    </div>
                    <div className="p-4">
                      <h3 className="font-bold text-[#0c4a6e] mb-1">{e.title}</h3>
                      <p className="text-xs text-gray-500 mb-1">Durée : {e.duration}</p>
                      <p className="text-xs text-[#c9a84c] font-semibold mb-3">Adulte : {e.priceAdult.toLocaleString()} FCFA · Enfant : {e.priceChild.toLocaleString()} FCFA</p>
                      <div className="flex gap-2">
                        <button onClick={() => toggleActive(e)}
                          className={`flex-1 flex items-center justify-center gap-1 text-xs font-bold py-2 rounded-lg transition-colors ${e.isActive ? "bg-orange-100 text-orange-600 hover:bg-orange-200" : "bg-green-100 text-green-600 hover:bg-green-200"}`}>
        ...
        {tab === "reservations" && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
              ...
              <h2 className="font-bold text-[#0c4a6e]">Réservations de voyages</h2>
              <ExportButton
                data={bookings.map(b => ({
                        ...
                        <tr key={b.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <VoucherButton data={{
                              ...
                              clientLastName: b.clientLastName,
                              clientPhone: b.clientPhone,
                              excursionTitle: b.excursion?.title ?? "Excursion",
                              bookingDate: b.bookingDate,
                              timeSlot: b.timeSlot,
                              adultsCount: b.adultsCount,
                              childrenCount: b.childrenCount,
                              guideLanguage: b.guideLanguage,
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 border border-gray-200 text-gray-600 font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition-colors">Annuler</button>
              <button onClick={saveExcursion} disabled={saving}
                className="flex-1 bg-[#c9a84c] hover:bg-[#b8973b] text-white font-bold py-2.5 rounded-xl transition-colors disabled:opacity-60">
