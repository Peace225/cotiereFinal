  ...
  async function loadData() {
    ...
    try {
      ...
      const [studioRes, eventRes, excursionRes, musicRes, equipmentRes] = await Promise.all([
        fetch("/api/studio/bookings?limit=500", fetchOptions),
        fetch("/api/events/requests?limit=500", fetchOptions),
        fetch("/api/excursions/bookings?limit=500", fetchOptions),
        fetch("/api/music/bookings?limit=500", fetchOptions),
        fetch("/api/equipment/rentals?limit=500", fetchOptions),
      ]);
      ...
      const studioBookings: Booking[] = (studioData.bookings ?? []).map((b: any) => ({ ...b, type: "studio" }));
      
      // ⚠️ CORRECTION FIABILITÉ : Vérifie explicitement que eventData est un tableau
      const rawEvents = eventData?.requests ?? eventData;
      ...
      const excursionBookings: Booking[] = (excursionData.bookings ?? []).map((b: any) => ({ ...b, type: "excursion", eventType: b.excursion?.title ?? "Excursion", eventDate: b.bookingDate }));
      const musicBookings: Booking[] = (musicData.bookings ?? []).map((b: any) => ({ ...b, type: "music", eventType: b.serviceType, eventDate: b.sessionDate }));
      const equipmentBookings: Booking[] = (equipmentData.rentals ?? []).map((b: any) => ({ ...b, type: "equipment", eventType: "Guichet Unique", eventDate: b.startDate }));
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button onClick={loadData}
              className="flex items-center gap-2 bg-[#c9a84c] hover:bg-[#b8973b] text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors cursor-pointer">
