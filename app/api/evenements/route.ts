...
// GET — liste publique des événements actifs
export async function GET(req: NextRequest) {
  try {
    const { searchParams } = new URL(req.url);
    const categorie = searchParams.get("categorie");
    const all = searchParams.get("all") === "true"; // admin: tous les événements

    const where = all ? {} : { isActive: true };
    if (categorie && categorie !== "Tous") {
      Object.assign(where, { categorie });
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      slug, titre, categorie, date, heure, lieu, ville, prix, capacite,
      organisateur, description, image, badge, badgeColor,
      artistes, duree, programme,
    } = body;
    ...
    const evenement = await prisma.evenement.create({
      data: {
        ...
        image: image || "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&q=80",
        badge: badge || "A venir",
        badgeColor: badgeColor || "bg-blue-500",
        artistes: artistes || null,
        duree: duree || null,
        programme: programme || [],
        isActive: true,
      },
