...
export default function AdminInfoPage() {
  const [prestations, setPrestations] = useState<Prestation[]>(DEFAULT_PRESTATIONS);
  ...
  function savePrestation() {
    const p: Prestation = {
      ...
      prix: form.prix,
      image: form.image || "/Images/cotiere-info.png",
      isActive: true,
    };
    if (editId) setPrestations(prev => prev.map(x => x.id === editId ? p : x));
    else setPrestations(prev => [...prev, p]);
              ...
              <div key={p.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                ...
                <div className="p-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setPrestations(prev => prev.map(x => x.id === p.id ? { ...x, isActive: !x.isActive } : x))}
                    ...
                    <button onClick={() => { if (confirm("Supprimer ?")) setPrestations(prev => prev.filter(x => x.id !== p.id)); }}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3 shrink-0">
              ...
              <button onClick={savePrestation} disabled={!form.nom}
