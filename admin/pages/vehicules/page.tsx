...
const DEFAULT_VEHICULES: Vehicule[] = [
  { id: "citadine", nom: "Voiture Citadine", categorie: "Économique", description: "Idéale pour les déplacements en ville. Climatisée, économique en carburant.", prix: "15 000 FCFA/jour", caution: "50 000 FCFA", places: 5, image: "https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?w=400&q=80", isActive: true },
  ...
  { id: "moto", nom: "Moto / Scooter", categorie: "Deux-roues", description: "Pratique pour se déplacer rapidement dans les ruelles et sur la côte.", prix: "8 000 FCFA/jour", caution: "30 000 FCFA", places: 2, image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80", isActive: true },
  ...
  { id: "minibus", nom: "Bus / Minibus", categorie: "Groupe", description: "Pour vos sorties en groupe, excursions et transferts d'équipes.", prix: "80 000 FCFA/jour", caution: "200 000 FCFA", places: 20, image: "https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?w=400&q=80", isActive: true },
  { id: "luxe", nom: "Véhicule de Luxe", categorie: "Premium", description: "Mercedes, BMW ou équivalent pour vos occasions spéciales.", prix: "100 000 FCFA/jour", caution: "500 000 FCFA", places: 5, image: "https://images.unsplash.com/photo-1555215695-3004980ad54e?w=400&q=80", isActive: true },
];

const emptyForm = { nom: "", categorie: "", description: "", prix: "", caution: "", places: "5", image: "" };

export default function AdminVehiculesPage() {
  const [vehicules, setVehicules] = useState<Vehicule[]>(DEFAULT_VEHICULES);
  ...
  function saveVehicule() {
    const v: Vehicule = {
      ...
      nom: form.nom, categorie: form.categorie, description: form.description,
      prix: form.prix, caution: form.caution, places: parseInt(form.places) || 5,
      image: form.image || "https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=400&q=80",
      isActive: true,
    };
    if (editId) setVehicules(prev => prev.map(x => x.id === editId ? v : x));
    else setVehicules(prev => [...prev, v]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-black text-[#0c4a6e]">Location de Véhicules</h1>
          </div>
            ...
            {vehicules.map(v => (
              <div key={v.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                ...
                <div className="p-4">
                  ...
                  <p className="text-xs text-gray-500 mb-1">{v.places} places · {v.prix}</p>
                  <p className="text-xs text-gray-400 mb-3">Caution : {v.caution}</p>
                  <div className="flex gap-2">
                    <button onClick={() => setVehicules(prev => prev.map(x => x.id === v.id ? { ...x, isActive: !x.isActive } : x))}
                      ...
                      <CheckCircle size={13} /> {v.isActive ? "Désactiver" : "Activer"}
                    </button>
                    ...
                    <button onClick={() => { if (confirm("Supprimer ?")) setVehicules(prev => prev.filter(x => x.id !== v.id)); }}
              ...
              <div className="ml-auto">
                <ExportButton
                  ...
                  columns={[
                    ...
                    { key: "telephone", label: "Téléphone" }, { key: "vehicule", label: "Véhicule" },
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>{["Réf.", "Client", "WhatsApp", "Véhicule demandé", "Date", "Statut", "Action"].map(h => (
                        <th key={h} className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase">{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {filtered.map(r => {
                        // Image par type de véhicule
                        ...
                        <tr key={r.id} className="hover:bg-gray-50">
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">
                              <img src={imgSrc} alt="véhicule" className="w-9 h-9 rounded-lg object-cover shrink-0 border border-gray-100" />
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
              <h3 className="font-bold text-[#0c4a6e] text-lg">{editId ? "Modifier" : "Ajouter"} un véhicule</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
            </div>
            <div className="p-6 space-y-4 overflow-y-auto">
              {[
                { key: "nom", label: "Nom du véhicule *", placeholder: "Voiture Citadine" },
                { key: "categorie", label: "Catégorie *", placeholder: "Économique / Tout-terrain / Premium..." },
              ...
              <div>
                ...
                <textarea rows={2} placeholder="Description du véhicule..." value={form.description}
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3 shrink-0">
              ...
              <button onClick={saveVehicule} disabled={!form.nom}
